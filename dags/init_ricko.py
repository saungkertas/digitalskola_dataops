...
with DAG('init_ricko',
    ...
    ingest_customers = BashOperator(
        ...
        bash_command="""python3 /root/airflow/dags/ingest/ricko/ingest_customers.py {{ execution_date.format('YYYY-MM-DD') }}"""
    )
    ...
    to_datalake_customers = BashOperator(
        ...
        bash_command="""gsutil cp /root/output/ricko/customers/customers_{{ execution_date.format('YYYY-MM-DD') }}.csv gs://digitalskola-de-batch7/ricko/staging/customers/"""
    )
    ...
    data_definition_orders = BashOperator(
        ...
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/ricko/staging/orders/* > /root/table_def/ricko/orders.def"""
    )
    ...
    data_definition_products = BashOperator(
        ...
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/ricko/staging/products/* > /root/table_def/ricko/products.def"""
    )

    data_definition_suppliers = BashOperator(
        task_id='data_definition_suppliers',
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/ricko/staging/suppliers/* > /root/table_def/ricko/suppliers.def"""
    )

    data_definition_categories = BashOperator(
        task_id='data_definition_categories',
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/ricko/staging/categories/* > /root/table_def/ricko/categories.def"""
    )

    data_definition_customers = BashOperator(
        task_id='data_definition_customers',
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/ricko/staging/customers/* > /root/table_def/ricko/customers.def"""
    )
    ...
    to_dwh_orders = BashOperator(
        ...
        bash_command="""bq mk --external_table_definition=/root/table_def/ricko/orders.def de_7.ricko_orders"""
    )
    ...
    to_dwh_products = BashOperator(
        ...
        bash_command="""bq mk --external_table_definition=/root/table_def/ricko/products.def de_7.ricko_products"""
    )

    to_dwh_suppliers = BashOperator(
        task_id='to_dwh_suppliers',
        bash_command="""bq mk --external_table_definition=/root/table_def/ricko/suppliers.def de_7.ricko_suppliers"""
    )

    to_dwh_categories = BashOperator(
        task_id='to_dwh_categories',
        bash_command="""bq mk --external_table_definition=/root/table_def/ricko/categories.def de_7.ricko_categories"""
    )

    to_dwh_customers = BashOperator(
        task_id='to_dwh_customers',
        bash_command="""bq mk --external_table_definition=/root/table_def/ricko/customers.def de_7.ricko_customers"""
    )
