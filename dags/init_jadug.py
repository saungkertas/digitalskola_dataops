...
for table in ['orders', 'order_details', 'products']:    
    ...
    data_definition = BashOperator(
        ...
        bash_command="""bq mkdef --autodetect --source_format=CSV gs://digitalskola-de-batch7/jadug/staging/{{ params.table_name }}/* > /root/table_def/jadug/{{ params.table_name }}.def""",
        params={'table_name':table}
    )
